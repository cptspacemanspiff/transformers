...
class SortformerFeatureExtractor(ParakeetFeatureExtractor):
    r"""
    ...
    (`normalize: "NA"`) used by the Sortformer diarization model. It shares the exact STFT / mel-filterbank pipeline of
    [`ParakeetFeatureExtractor`], differing only in the number of mel bins (128 vs 80) and in that it does **not** apply
    per-feature mean/variance normalization by default (Sortformer is trained with `normalize: "NA"`).
    ...
    def __init__(
        ...
        **super_kwargs,
        ...
        super().__init__(feature_size=feature_size, **super_kwargs)
