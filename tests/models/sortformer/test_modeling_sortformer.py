...
class SortformerStreamingConformerTest(unittest.TestCase):
    ...
    def _build_streaming_model(self, fifo_len=20, spkcache_len=16):
        """A model configured with small AOSC buffers (fifo_len >= spkcache_len, as in the real config) so the circular
        FIFO fills and the speaker cache periodically re-encodes."""
        config, model = self._build_model()
        config.chunk_len = 4
        config.fifo_len = fifo_len
        config.spkcache_len = spkcache_len  # multiple of num_speakers, > num_speakers * spkcache_sil_frames_per_spk
        config.chunk_left_context = 1
        config.chunk_right_context = 2
        config.spkcache_sil_frames_per_spk = 0
        return config, model

    def test_streaming_step_incremental_shape(self):
        """The incremental streaming path returns one prediction per output frame, like `diarize_streaming`."""
        config, model = self._build_streaming_model()
        torch.manual_seed(0)
        features = torch.randn(1, 8 * 30, config.encoder_config.num_mel_bins, device=torch_device)
        with torch.no_grad():
            preds = model.diarize_streaming_incremental(features, right_context=7, compression_period=8)
        # one subsampled output frame per `subsampling_factor` input frames
        self.assertEqual(preds.shape, (1, 30, config.num_speakers))

    def test_incremental_no_compression_matches_streaming_diarize(self):
        """With a FIFO large enough that nothing slides out and no compression ever fires, the incremental encode
        reduces to the plain cache-aware single pass, so it must match `streaming_diarize` (same causal computation)."""
        config, model = self._build_streaming_model(fifo_len=10_000, spkcache_len=10_000)
        torch.manual_seed(0)
        features = torch.randn(1, 8 * 20, config.encoder_config.num_mel_bins, device=torch_device)
        with torch.no_grad():
            incremental = model.diarize_streaming_incremental(features, right_context=7, compression_period=10_000)
            reference = model.streaming_diarize(features, right_context=7).sigmoid()
        self.assertEqual(incremental.shape, reference.shape)
        # only the per-chunk vs whole-utterance convolutional subsampling differs at chunk boundaries
        self.assertLess((incremental - reference).abs().max().item(), 5e-3)

    def test_incremental_periodic_reencode_bounded(self):
        """The FIFO stays full and bounded, the speaker cache never exceeds `spkcache_len`, and the periodic re-encode
        fires more than once (so the select-then-re-encode path is exercised)."""
        config, model = self._build_streaming_model()
        torch.manual_seed(0)
        features = torch.randn(1, 8 * 60, config.encoder_config.num_mel_bins, device=torch_device)

        feat_seq = features.transpose(1, 2)
        feat_len = torch.tensor([features.shape[1]], device=torch_device)
        state = model.init_streaming_state_incremental(1, device=torch_device)
        reencodes = 0
        steady_fifo = []
        with torch.no_grad():
            for chunk_features, chunk_lengths, left_offset, right_offset in model._streaming_feat_loader(
                feat_seq, feat_len
            ):
                _, state, op = model.streaming_step_incremental(
                    chunk_features, state, chunk_lengths, left_offset, right_offset, right_context=7,
                    compression_period=8,
                )
                reencodes += int(op["compressed"])
                self.assertLessEqual(op["spkcache"], config.spkcache_len)
                self.assertLessEqual(op["fifo"], config.fifo_len)
                steady_fifo.append(op["fifo"])
        self.assertGreater(reencodes, 1, "test clip did not trigger repeated periodic re-encodes")
        # once warmed up the circular FIFO is pinned full
        self.assertEqual(steady_fifo[-1], config.fifo_len)
